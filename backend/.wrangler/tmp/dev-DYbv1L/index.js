...
// .wrangler/tmp/bundle-bUV237/checked-fetch.js
...
app.post("/login", async (c) => {
  ...
  try {
    ...
    const testKey = await crypto.subtle.deriveBits(
      { name: "PBKDF2", salt: Buffer.from(salt, "hex"), iterations: 1e5, hash: "SHA-256" },
      await crypto.subtle.importKey("raw", new TextEncoder().encode(password), { name: "PBKDF2" }, false, ["deriveBits"]),
      256
    );
    if (Buffer.from(testKey).toString("hex") !== storedHash) {
      return c.json({ error: "Credenciales inv\xE1lidas" }, 401);
    }
    const secret = new TextEncoder().encode(c.env.JWT_SECRET);
    const token = await new SignJWT({ userId: user.id }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("2h").sign(secret);
    return c.json({ token });
  } catch (error) {
    return c.json({ error: "Error del servidor" }, 500);
  }
...
// .wrangler/tmp/bundle-bUV237/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bUV237/middleware-loader.entry.ts
