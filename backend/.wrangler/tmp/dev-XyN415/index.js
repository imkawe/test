...
// .wrangler/tmp/bundle-tGVOLS/checked-fetch.js
...
async function getPayPalAccessToken() {
  const auth2 = Buffer.from(PAYPAL_CLIENT_ID + ":" + PAYPAL_SECRET).toString("base64");
...
var createPayPalOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    if (paypalData.error)
      throw new Error(paypalData.error.message);
    const order_id = v4_default();
    const order = await c.env.DB.prepare(`
      INSERT INTO orders (
        order_id, 
...
var completeOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    throw new HTTPException(500, { message: "Error al completar orden" });
  }
}, "completeOrder");
...
// .wrangler/tmp/bundle-tGVOLS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-tGVOLS/middleware-loader.entry.ts
