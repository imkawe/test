...
// .wrangler/tmp/bundle-W9QWGT/checked-fetch.js
  ...
  try {
    ...
    const products = await Promise.all(
      body.items.map(async (item) => {
        if (!item.product_id || !item.quantity || item.unit_price === void 0) {
          throw new HTTPException(400, { message: "Invalid product data" });
        }
        const product = await c.env.DB.prepare(
          `SELECT id, name, image, price, stock FROM products WHERE id = ?`
    ...
    console.log("Insertando productos en order_products:", products);
    for (const product of products) {
      ...
      console.log(`Producto ${product.product_id} insertado correctamente`);
    }
    ...
    for (const item of body.items) {
      ...
      console.log(`Stock actualizado para producto ${item.product_id}`);
    }
  ...
  try {
    ...
    const ordersWithProducts = await c.env.DB.prepare(
      `SELECT 
        o.id AS order_id,
        o.user_id,
        u.name AS user_name, -- \u{1F539} Se agrega el nombre del usuario
        u.email AS user_email, -- Opcional, si quieres mostrar el email
        o.status,
        o.total_amt,
        o.created_at,
        op.product_id,
      ...
      FROM 
        orders o
      LEFT JOIN 
        users u ON o.user_id = u.id -- \u{1F539} Se une con la tabla users
      LEFT JOIN 
  ...
  try {
    ...
    const productsResult = await c.env.DB.prepare(
      `SELECT 
        ...
        op.quantity,
        op.total_price,
        op.discount -- Se agrega el campo descuento
      FROM 
        order_products op
      LEFT JOIN 
        products p ON op.product_id = p.id
    ...
    const order = {
      ...orderResult,
      // Mantenemos todos los datos de la orden
      items: productsResult.results.map((row) => ({
        ...
        // Asegura que el descuento sea un número válido
        total_price: row.total_price
      }))
    };
    return c.json({ success: true, data: order });
...
// .wrangler/tmp/bundle-W9QWGT/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-W9QWGT/middleware-loader.entry.ts
