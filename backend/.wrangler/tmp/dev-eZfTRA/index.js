...
// .wrangler/tmp/bundle-K1SEIf/checked-fetch.js
...
var createAddress = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const { meta } = await db(c.env).prepare(
      ...
      body.city,
      body.state || "",
      // Usar string vacío en lugar de null
      body.pincode,
      body.country,
      body.mobile,
      user.id
...
var PAYPAL_CLIENT_ID = "ATy-YTAw2BnbshPqRimptGmp2hxFrAC5DcRb-3r7VifaE1gyrChXEqIeWUbkQZPl-UVXbshJhiG-murT";
var PAYPAL_SECRET = "EEranYhXnr5qN_I3bULbzEiDt87z7IT02sefb5IOj1q2kiZctGLnBCxL36gbgQHzUGmefj19W5jkvhV5";
async function getPayPalAccessToken() {
  const auth2 = btoa(`${PAYPAL_CLIENT_ID}:${PAYPAL_SECRET}`);
...
var createPayPalOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const products = await Promise.all(
      body.items.map(async (item) => {
        if (!item.product_id)
          throw new HTTPException(400, { message: "product_id es requerido" });
        const product = await c.env.DB.prepare(`
          SELECT id, price, stock FROM products WHERE id = ?
        `).bind(item.product_id).first();
        if (!product)
    ...
    if (Math.abs(subTotal - parseFloat(body.amount)) > 0.01) {
      throw new HTTPException(400, { message: "El monto total no coincide con los items" });
    }
    const accessToken = await getPayPalAccessToken();
    const paypalOrderId = v4_default();
    ...
    if (!paypalResponse.ok)
      throw new HTTPException(502, {
        message: `Error PayPal: ${paypalData?.details?.[0]?.description || "Unknown error"}`
      });
    const order = await c.env.DB.prepare(`
      INSERT INTO orders (
        order_id, 
    ...
    for (const product of products) {
      ...
      await c.env.DB.prepare(`
        ...
        SET stock = stock - ? 
        WHERE id = ?
      `).bind(product.quantity, product.product_id).run();
    }
    return c.json({
      success: true,
      data: {
        ...order,
        paypal_approval_url: paypalData.links.find((l) => l.rel === "approve")?.href
      }
    });
  } catch (error) {
    console.error("Error completo:", error);
...
var completeOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    if (paypalData.status !== "COMPLETED") {
      throw new HTTPException(402, { message: "Pago no completado" });
    }
    await c.env.DB.prepare(`
      UPDATE orders 
...
var getOrders = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const ordersWithProducts = await c.env.DB.prepare(
      ...
      LEFT JOIN addresses a ON o.delivery_address = a.id -- \u{1F539} Se une con la tabla addresses
      ...
      if (order) {
        ...
        acc.push({
          ...
          address: row.address_id ? {
            // 🔹 Agregar la dirección solo si existe
            id: row.address_id,
...
// .wrangler/tmp/bundle-K1SEIf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-K1SEIf/middleware-loader.entry.ts
