...
// .wrangler/tmp/bundle-ucjNiJ/checked-fetch.js
...
var getProductById = /* @__PURE__ */ __name(async (c) => {
  ...
  try {
    const { results } = await db(c.env).prepare("SELECT * FROM products WHERE id = ?").all(id);
...
var updateProduct = /* @__PURE__ */ __name(async (c) => {
  ...
  try {
    const { name, image, category_id, unit, stock, price, discount, description, more_details, publish } = await c.req.json();
    const result = await db(c.env).prepare(`
      UPDATE products SET 
        name = ?, 
        image = ?, 
        category_id = ?, 
      ...
      WHERE id = ?
    `).run(name, JSON.stringify(image), category_id, unit, stock, price, discount, description, JSON.stringify(more_details), publish, id);
    if (result.changes === 0) {
      return c.json({ error: "Producto no encontrado" }, 404);
    }
...
var deleteProduct = /* @__PURE__ */ __name(async (c) => {
  const id = c.req.param("id");
  try {
    const result = await db(c.env).prepare("DELETE FROM products WHERE id = ?").run(id);
    if (result.changes === 0) {
      return c.json({ error: "Producto no encontrado" }, 404);
    }
...
// .wrangler/tmp/bundle-ucjNiJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ucjNiJ/middleware-loader.entry.ts
