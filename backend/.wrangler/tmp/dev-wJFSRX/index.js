...
// .wrangler/tmp/bundle-NkqiTG/checked-fetch.js
...
var createOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const body = await c.req.json();
    if (!body.delivery_address || !body.items?.length) {
    ...
    const products = await Promise.all(
      body.items.map(async (item) => {
        ...
        const total = price * item.quantity * (1 - discount);
        subTotal += total;
        return {
    ...
    const orderData = await c.env.DB.prepare(
      `INSERT INTO orders (
        user_id, delivery_address, sub_total_amt, total_amt, 
      ...
      subTotal,
      subTotal,
      // Aquí podrías agregar impuestos/envío
      body.metodo_de_pago,
      "pending",
      "pending"
    ).first();
...
// .wrangler/tmp/bundle-NkqiTG/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-NkqiTG/middleware-loader.entry.ts
