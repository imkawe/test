...
// .wrangler/tmp/bundle-99eG1f/checked-fetch.js
    ...
    const products = await Promise.all(
      body.items.map(async (item) => {
        ...
        return {
          ...
          discount,
          total_price: total,
          more_details: JSON.stringify(moreDetails)
          // Convertir a JSON
        };
      })
    );
      ...
      if (order) {
        if (row.product_id) {
          order.items.push({
            ...
            more_details: row.more_details ? JSON.parse(row.more_details) : null
            // 🔹 Parsear
          });
        }
      } else {
...
// .wrangler/tmp/bundle-99eG1f/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-99eG1f/middleware-loader.entry.ts
