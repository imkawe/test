...
// .wrangler/tmp/bundle-pJd1tA/checked-fetch.js
...
var createPayPalOrder = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    for (const item of body.items) {
      ...
      await c.env.DB.prepare(
        `INSERT INTO order_products (order_id, product_id, quantity, unit_price, discount, total_price)
         VALUES (?, ?, ?, ?, ?, ?)`
      ).bind(
        order_id,
        item.product_id,
...
var cancelarOrdenesPendientes = /* @__PURE__ */ __name(async (env) => {
  try {
    console.log("\u23F3 Revisando \xF3rdenes pendientes de PayPal...");
    const cutoffTime = new Date(Date.now() - 30 * 60 * 1e3).toISOString().replace("T", " ").replace(/\.\d+Z$/, "");
    const { success, meta } = await env.DB.prepare(`
      ...
      WHERE 
        status = 'PENDING' AND
        payment_method = 'PayPal' AND
        created_at < ? AND
        payment_id != ''
    `).bind(cutoffTime).run();
    console.log(`\u2705 ${meta.changes} \xF3rdenes canceladas. Tiempo l\xEDmite: ${cutoffTime}`);
    return meta.changes;
  } catch (error) {
    console.error("\u274C Error al cancelar \xF3rdenes:", error);
    throw new Error(`Error cancelando \xF3rdenes: ${error.message}`);
  }
...
// .wrangler/tmp/bundle-pJd1tA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-pJd1tA/middleware-loader.entry.ts
