...
// .wrangler/tmp/bundle-8A2Lit/checked-fetch.js
...
var db = {
  prepare: (query) => ({
    all: async () => {
      const { results } = await (await env.DB.prepare(query)).all();
      return { results };
    },
    run: async (...params) => {
...
var getProducts = /* @__PURE__ */ __name(async (c) => {
  try {
    const { results } = await (await db.prepare("SELECT * FROM products")).all();
    return c.json(results);
  } catch (error) {
...
var getProductById = /* @__PURE__ */ __name(async (c) => {
  ...
  try {
    const { results } = await (await db.prepare("SELECT * FROM products WHERE id = ?")).all(id);
...
var addProduct = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    await (await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)).run(name, image, category_id, unit, stock, price, discount, description, more_details, publish);
    return c.json({ message: "Producto a\xF1adido correctamente" });
  } catch (error) {
    console.error("\u{1F6A8} Error al a\xF1adir producto:", error);
...
var updateProduct = /* @__PURE__ */ __name(async (c) => {
  ...
  try {
    const { name, image, category_id, unit, stock, price, discount, description, more_details, publish } = await c.req.json();
    await (await db.prepare(`
      UPDATE products SET name = ?, image = ?, category_id = ?, unit = ?, stock = ?, 
      price = ?, discount = ?, description = ?, more_details = ?, publish = ?, 
      updated_at = CURRENT_TIMESTAMP WHERE id = ?
    `)).run(name, image, category_id, unit, stock, price, discount, description, more_details, publish, id);
...
var deleteProduct = /* @__PURE__ */ __name(async (c) => {
  const id = c.req.param("id");
  try {
    await (await db.prepare("DELETE FROM products WHERE id = ?")).run(id);
...
// .wrangler/tmp/bundle-8A2Lit/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-8A2Lit/middleware-loader.entry.ts
